...
test.describe("AdminLanguageSwitcher — bascule de langue E2E", () => {
  // NextAuth v5 beta utilise un CSRF token global au serveur. Quand plusieurs
  // workers Playwright se connectent en parallèle sur le même serveur dev, les
  // tokens CSRF s'invalident mutuellement → login flaky ("MissingCSRF").
  // On sérialise donc tous les tests de ce fichier (qui font du login admin)
  // pour qu'ils tournent dans un seul worker, séquentiellement.
