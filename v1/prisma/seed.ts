...
async function main() {
  ...
  // Récupération des communes existantes AVANT la boucle d'upsert : sert à la
  // fois à l'audit des codes réécrits (nom différent) et au nettoyage des
  // communes obsolètes (codes absents du nouveau dataset) — une seule requête.
  const communesExistantes = await prisma.commune.findMany({ select: { code: true, name: true } });
  const communesParCode = new Map(communesExistantes.map((c) => [c.code, c.name]));
  const codesReecrits: { code: string; ancienNom: string; nouveauNom: string }[] = [];
  ...
  for (const wilaya of wilayas) {
    ...
    if (wilaya.communes) {
      for (const commune of wilaya.communes) {
        // Audit : les codes positionnels sont réutilisés entre datasets — un code
        // peut pointer vers une commune différente. Information d'audit, pas une erreur.
        const ancienNom = communesParCode.get(commune.code);
        if (ancienNom !== undefined && ancienNom !== commune.name) {
          codesReecrits.push({ code: commune.code, ancienNom, nouveauNom: commune.name });
        }

        await prisma.commune.upsert({
          where: { code: commune.code },
          // wilayaCode dans update : défensif — les communes déplacées (Loi 26-06)
          // changent de code donc sont recréées, mais la cohérence du rattachement
          // doit être garantie même si un code venait à être réutilisé.
          update: { name: commune.name, nameAr: commune.nameAr, wilayaCode: wilaya.code },
          create: {
            code: commune.code,
            name: commune.name,
  ...
  if (codesReecrits.length > 0) {
    const exemples = codesReecrits
      .slice(0, 5)
      .map((c) => `  ${c.code} : ${c.ancienNom} → ${c.nouveauNom}`)
      .join("\n");
    console.warn(
      `⚠️ ${codesReecrits.length} code(s) de commune réécrit(s) (nom différent de l'ancien dataset) :\n${exemples}`
    );
  }

  // Nettoyage des communes obsolètes : codes absents du nouveau dataset.
  // Correction nécessaire car l'ancien dataset contenait des communes erronées
  // (ex. Timimoun rattachée à Adrar alors qu'elle est la wilaya 49).
  // On ne supprime JAMAIS les wilayas.
  const codesValides = new Set<string>();
  for (const wilaya of wilayas) {
    for (const commune of wilaya.communes ?? []) {
      codesValides.add(commune.code);
    }
  }

  const codesObsoletes = communesExistantes
    .map((c) => c.code)
    .filter((code) => !codesValides.has(code));

  // Suppression par lots (évite tout risque de limite de paramètres SQL)
  let communesSupprimees = 0;
  for (let i = 0; i < codesObsoletes.length; i += 1000) {
    const lot = codesObsoletes.slice(i, i + 1000);
    const resultat = await prisma.commune.deleteMany({ where: { code: { in: lot } } });
    communesSupprimees += resultat.count;
  }

  if (communesSupprimees > 0) {
    console.log(`🧹 ${communesSupprimees} communes obsolètes supprimées (ancien dataset)`);
  } else {
    console.log("🧹 Aucune commune obsolète à supprimer");
  }
